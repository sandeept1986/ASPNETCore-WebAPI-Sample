...
namespace SampleWebApiAspNetCore.v1.Controllers
{
    ...
    public class FoodsController : ControllerBase
    {
        ...
        //test
        [HttpGet]
        [Route("GetByCalorieRange/{min:int}-{max:int}", Name = nameof(GetByCalorieRange))]
        public ActionResult GetByCalorieRange([FromQuery] QueryParameters queryParameters, int min, int max)
        {
            //List<FoodItem> foodItems = _foodRepository.GetByCalories(queryParameters, min, max).ToList();
            List<FoodItem> foodItems = _foodRepository.GetAll(queryParameters).ToList();
            List<FoodItem> foodsInRange = new List<FoodItem>();
            for (int i = 0;  i < foodItems.Count; i++)
            {
                if (foodItems[i].Calories >= min && foodItems[i].Calories <= max)
                {
                    foodsInRange.Add(foodItems[i]);
                }
                
            }
            //var allItemCount = _foodRepository.Count();
            var allItemCount = foodsInRange.Count();
            ...
            var toReturn = foodsInRange.Select(x => ExpandSingleFoodItem(x));
